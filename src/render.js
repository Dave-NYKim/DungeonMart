...
function drawMonster(c,e,time){
 if(e.boss){const sprite=bossSprite(Math.floor(time*2+e.x)%2,e.facing??0),x=Math.round(e.x),y=Math.round(e.y),sw=sprite.width,sh=sprite.height,top=y-(sprite.anchorY??sh-4);oval(c,x,y+2,Math.round(sw*.4),Math.round(sw*.11),'#18241d99');c.imageSmoothingEnabled=false;
...
export class WorldRenderer {
 ...
 draw(s,selected,selectedZone){
  ...
  for(const h of s.heroes){const prev=this.previous.get(h.id),moved=prev?Math.hypot(h.x-prev.x,h.y-prev.y)>.03:false,movingUntil=moved?s.time+.2:prev?.movingUntil||0,target=s.enemies.find(e=>e.id===h.target),flip=target?target.x<h.x:moved&&prev&&Math.abs(h.x-prev.x)>.01?h.x<prev.x:prev?.flip||false;const facing=target?facingTo(h,target):moved&&prev?facingTo(prev,h):prev?.facing??0;const attackStarted=prev&&h.attacks>prev.attacks?s.time:prev?.attackStarted??-10;this.previous.set(h.id,{x:h.x,y:h.y,flip,facing,movingUntil,attacks:h.attacks,attackStarted});if(!visible(h.x,h.y))continue;
   entities.push({y:h.y,draw:()=>{if(h.state==='arrive'&&h.arrivalStage===-1)l.drawImage(this.boat,h.x-82,h.y-90);for(const p of h.pets){l.globalAlpha=.7;drawMonster(l,{...p,type:p.kind==='golem'?'hulk':'skeleton',hp:1,maxHp:1},s.time);l.globalAlpha=1;}drawHero(l,h,s,Math.round(h.x),Math.round(h.y),.5,s.time,false,{moving:s.time<movingUntil,flip,facing,attack:h.state==='hunt'&&s.time-attackStarted<.8,frame:h.state==='hunt'&&s.time-attackStarted<.8?Math.min(7,Math.floor((s.time-attackStarted)*10)):undefined});if(h.hp<statsOf(h,s).hp*.8){const x=Math.round(h.x),y=Math.round(h.y-(h.classId==='barbarian'?68:38));panel(l,x-10,y,21,4,'#2c3732');rect(l,x-9,y+1,Math.max(0,Math.round(19*h.hp/statsOf(h,s).hp)),2,h.hp<statsOf(h,s).hp*.3?'#d77c73':'#b6d185');}}});
  }
  ...
  for(const f of s.effects)if(visible(f.x,f.y)){const a=f.life/f.max;l.globalAlpha=a;if(f.kind==='skill'){const sk=skillById(f.skillId);if(sk)drawSkillEffect(l,sk,f.from,f.to,1-a,f.targets.length?f.targets:[f.to]);}else if(f.kind==='line'){stroke(l,f.x,f.y,f.to.x,f.to.y,f.color);rect(l,f.to.x-1,f.to.y-1,3,3,'#ffebbc');}else if(f.kind==='dark'){const t=1-a,ex=f.x+(f.to.x-f.x)*Math.min(1,t*2.2),ey=f.y+(f.to.y-f.y)*Math.min(1,t*2.2);stroke(l,f.x,f.y,ex,ey,'#3b1560',5);stroke(l,f.x,f.y,ex,ey,'#8a4dff',3);stroke(l,f.x,f.y,ex,ey,'#e2ccff',1);for(let i=0;i<6;i++){const k=(i+1)/7,px=f.x+(ex-f.x)*k,py=f.y+(ey-f.y)*k,j=Math.sin(t*40+i*2.1)*6;rect(l,px-j*.4,py+j,2,2,i%2?'#c39bff':'#5b2a8a');}rect(l,ex-2,ey-2,5,5,'#f0e4ff');}else if(f.kind!=='text'){const radius=5+(1-a)*24;for(let i=0;i<8;i++){const angle=i*Math.PI/4;rect(l,f.x+Math.cos(angle)*radius,f.y+Math.sin(angle)*radius*.55,2,2,f.color);}}}l.globalAlpha=1;
  ...
  else{for(const p of POIS)if(visible(p.x,p.y))label(p.name+(p.status==='reserved'?' · 확장 예정':''),p.x,p.y+30,p.status==='reserved'?'#c4c6a4':'#ead29b',10,true);const nameLabels=[];for(const h of s.heroes){if(zoom<.85)continue;const pos=this.camera.screenPoint(h.x,h.y-(h.classId==='barbarian'?74:44));if(nameLabels.some(p=>Math.abs(p.x-pos.x)<58&&Math.abs(p.y-pos.y)<18))continue;nameLabels.push(pos);label(h.name+(h.state==='arrive'?' · 입장 중':''),h.x,h.y-(h.classId==='barbarian'?74:44),HERO_GRADES[h.grade ?? 0].color,11);}}
