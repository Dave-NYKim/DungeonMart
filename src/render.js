import {actBossSprite,campaignArtRevision} from './campaign-art.js';
import { drawSummon } from './summon-art.js';
import { personalTint } from './hero-identity.js';
import { nightmareSprite } from './nightmare-art.js';
import { drawBossWarning, drawBossEffect } from './boss-effects.js';
import { HERO_GRADES, DIFFICULTIES } from './data.js';
import { ZONES, MONSTERS, MART, classOf } from './data.js';
import { statsOf, availableZone, gearSummary, facingTo } from './engine.js';
import { GRADES, itemBase, gradeColor, displayName } from './items.js';
import { canvasOf, rect, oval, poly, stroke, panel, pixelText, heroSprite, monsterSprite, treeSprite, stoneSprite, martSprite, shade, scale2x, bossSprite } from './pixel-art.js';
...
export function appearanceOf(h, s) {
  ...
  return { body:cls.id,tint:palette[h.costume.palette]||personalTint(h),color:palette[h.costume.palette]||(g.body?gradeColor(g.body):heroIdentity(h).accent),armor,weapon,weaponTier:g.weapon?GRADES[g.weapon.grade].sprite:-1,glow:g.accessory?GRADES[g.accessory.grade].sprite:-1,glowColor:g.accessory?gradeColor(g.accessory):null,tier:h.path.length,variant:Number(h.id.slice(1))%3,accent:heroIdentity(h).accent };
}
export function drawHero(c,h,s,x,y,scale=1,time=0,selected=false,pose={}) {
 if(h.state==='dead'){c.save();const bob=Math.sin(time*3)*3*scale;c.translate(x,y-18*scale+bob);c.scale(scale,scale);c.globalAlpha=.8;c.fillStyle='#a4e6e5';c.beginPath();c.moveTo(-12,12);c.quadraticCurveTo(-17,-20,0,-23);c.quadraticCurveTo(17,-20,12,12);c.lineTo(6,7);c.lineTo(0,13);c.lineTo(-6,7);c.closePath();c.fill();c.fillStyle='#244c63';c.fillRect(-7,-9,4,6);c.fillRect(3,-9,4,6);c.restore();return;}

  const a=appearanceOf(h,s),moving=pose.moving??['depart','return'].includes(h.state),frame=pose.frame??(moving||pose.attack?Math.floor(time*8+Number(h.id.slice(1))):0);
  const sprite=heroSprite(a,frame,pose.attack||false,pose.facing??0,moving),size=2*scale;
  const sw=sprite.width,sh=sprite.height,ax=sprite.anchorX??12,ay=sprite.anchorY??29;
  c.save();c.imageSmoothingEnabled=false;
  if(h.state==='dead')c.globalAlpha=.4;
  oval(c,Math.round(x),Math.round(y+3*scale),sprite.directional?sw*size*.3:11*scale,sprite.directional?5*size:3*scale,'#15251b88');
  ...
  if(pose.flip&&!sprite.directional){c.translate(Math.round(x),0);c.scale(-1,1);c.drawImage(sprite,-Math.round(ax*size),Math.round(y-ay*size),Math.round(sw*size),Math.round(sh*size));}
  else c.drawImage(sprite,Math.round(x-ax*size),Math.round(y-ay*size),Math.round(sw*size),Math.round(sh*size));
  c.restore();
}
function drawMonster(c,e,time){
 if(e.boss){const sprite=(Number.isInteger(e.actBoss)?actBossSprite(e.actBoss):null)||bossSprite(Math.floor(time*2+e.x)%2,e.facing??0),x=Math.round(e.x),y=Math.round(e.y),sw=sprite.width,sh=sprite.height,top=y-(sprite.anchorY??sh-4);oval(c,x,y+2,Math.round(sw*.4),Math.round(sw*.11),'#18241d99');c.imageSmoothingEnabled=false;
  if(e.spin){// Dark spin: a violet rune ring races around the feet while the boss turns.
   oval(c,x,y+2,Math.round(sw*.46),Math.round(sw*.13),'#2a123f88');for(let i=0;i<14;i++){const ph=time*6+i*Math.PI*2/14,rr=sw*.46;rect(c,x+Math.round(Math.cos(ph)*rr),y+2+Math.round(Math.sin(ph)*rr*.28)-Math.round(Math.abs(Math.sin(ph*1.5))*10),3,3,i%2?'#c39bff':'#7a3cff');}}
  for(let i=0;i<7;i++){const ph=time*1.3+i*.9;rect(c,x+Math.round(Math.cos(ph)*sw*.47+Math.sin(ph*2.3)*6),top+Math.round(sh*.7)+Math.round(Math.sin(ph*1.7)*sh*.22),2,2,i%2?'#ffb457':'#ff7a2e');}
  c.drawImage(sprite,x-Math.floor(sw/2),top);const bw=Math.max(49,Math.round(sw*.5));panel(c,x-Math.floor(bw/2),top-12,bw+1,6,'#382f31');rect(c,x-Math.floor(bw/2)+1,top-11,Math.round((bw-1)*Math.max(0,e.hp/e.maxHp)),4,e.hp<e.maxHp*.3?'#ff6a4a':'#e4b875');return;}
 const m=MONSTERS[e.type],size=e.elite?1.25:.85,x=Math.round(e.x),y=Math.round(e.y);
 {
  const sprite=nightmareSprite(e.type,e.curse>0,e.tier??0,e._attackPose>0||e._casting>0?1:e._moving?Math.floor(time*7+e.x)%2:0);
  if(sprite){const side=64*size,bob=e._moving?Math.floor(time*7)%2:0,top=y-60*size-bob;
   if(e.specialCd<=1&&e.specialCd>0&&!e.stun){c.save();c.globalAlpha=.35+(1-e.specialCd)*.35;c.strokeStyle=m.color;c.lineWidth=2;c.beginPath();c.ellipse(x,y,26*size,10*size,0,0,Math.PI*2);c.stroke();c.restore();}
   oval(c,x,y+1,14*size,4*size,'#18241d88');c.imageSmoothingEnabled=false;c.save();c.translate(x+(e._attackPose>0?(e._facing||1)*4:0),0);c.scale(e._facing===-1?-1:1,1);c.drawImage(sprite,Math.round(-side/2),Math.round(top),Math.round(side),Math.round(side));c.restore();
   if(e.elite||e.hp<e.maxHp){panel(c,x-10,top-6,21,4,'#382f31');rect(c,x-9,top-5,Math.round(19*Math.max(0,e.hp/e.maxHp)),2,e.elite?'#e4b875':'#c57e72');}return;}
 }
...
export class WorldRenderer {
 ...
 rebuild(s){const scene=buildScene(s.town);this.bg=scene.background;this.props=scene.props;this.flats=scene.flats;this.sceneRevision=layoutRevision;this.artRevision=campaignArtRevision;}
 draw(s,selected,selectedZone){
  if(this.sceneRevision!==layoutRevision||this.artRevision!==campaignArtRevision)this.rebuild(s);
  ...
  for(const f of this.flats)if(visible(f.x,f.y,f.arena?440:40))l.drawImage(f.sprite,f.x-(f.arena?f.sprite.width/2:16),f.y-(f.arena?f.sprite.height-8:16));
  ...
  for(const e of s.enemies)if(e.boss&&visible(e.x,e.y,250))drawBossWarning(l,e);
  ...
  for(const h of s.heroes){const prev=this.previous.get(h.id),moved=prev?Math.hypot(h.x-prev.x,h.y-prev.y)>.03:false,movingUntil=moved?s.time+.2:prev?.movingUntil||0,target=s.enemies.find(e=>e.id===h.target),flip=target?target.x<h.x:moved&&prev&&Math.abs(h.x-prev.x)>.01?h.x<prev.x:prev?.flip||false;const facing=target?facingTo(h,target):moved&&prev?facingTo(prev,h):prev?.facing??0;const attackStarted=prev&&h.attacks>prev.attacks?s.time:prev?.attackStarted??-10;this.previous.set(h.id,{x:h.x,y:h.y,flip,facing,movingUntil,attacks:h.attacks,attackStarted});if(!visible(h.x,h.y))continue;
   entities.push({y:h.y,draw:()=>{if(h.state==='arrive'&&h.arrivalStage===-1)l.drawImage(this.boat,h.x-82,h.y-90);for(const p of h.pets){if(drawSummon(l,p,s.time))continue;l.globalAlpha=.7;drawMonster(l,{...p,type:p.kind==='golem'?'hulk':'skeleton',hp:1,maxHp:1},s.time);l.globalAlpha=1;}drawHero(l,h,s,Math.round(h.x),Math.round(h.y),.5,s.time,false,{moving:s.time<movingUntil,flip,facing,attack:h.state==='hunt'&&s.time-attackStarted<.8,frame:h.state==='hunt'&&s.time-attackStarted<.8?Math.min(7,Math.floor((s.time-attackStarted)*10)):undefined});if(h.hp<statsOf(h,s).hp*.8){const x=Math.round(h.x),y=Math.round(h.y-(['barbarian','sorceress','necromancer','amazon','paladin'].includes(h.classId)?68:38));panel(l,x-10,y,21,4,'#2c3732');rect(l,x-9,y+1,Math.max(0,Math.round(19*h.hp/statsOf(h,s).hp)),2,h.hp<statsOf(h,s).hp*.3?'#d77c73':'#b6d185');}}});
  }
  ...
  for(const f of s.effects)if(visible(f.x,f.y,260)){const a=f.life/f.max;l.globalAlpha=a;if(f.kind==='skill'){const sk=skillById(f.skillId);if(sk)drawSkillEffect(l,sk,f.from,f.to,1-a,f.targets.length?f.targets:[f.to]);}else if(f.kind==='line'){stroke(l,f.x,f.y,f.to.x,f.to.y,f.color);rect(l,f.to.x-1,f.to.y-1,3,3,'#ffebbc');}else if(['dark','boss-blast','boss-summon'].includes(f.kind)){drawBossEffect(l,f);}else if(f.kind!=='text'){const radius=5+(1-a)*24;for(let i=0;i<8;i++){const angle=i*Math.PI/4;rect(l,f.x+Math.cos(angle)*radius,f.y+Math.sin(angle)*radius*.55,2,2,f.color);}}}l.globalAlpha=1;
  ...
  else{for(const p of POIS)if(visible(p.x,p.y))label(p.name+(p.status==='reserved'?' · 확장 예정':''),p.x,p.y+30,p.status==='reserved'?'#c4c6a4':'#ead29b',10,true);const nameLabels=[];for(const h of s.heroes){if(zoom<.85)continue;const pos=this.camera.screenPoint(h.x,h.y-(['barbarian','sorceress','necromancer','amazon','paladin'].includes(h.classId)?74:44));if(nameLabels.some(p=>Math.abs(p.x-pos.x)<58&&Math.abs(p.y-pos.y)<18))continue;nameLabels.push(pos);label(h.name+(h.state==='arrive'?' · 입장 중':''),h.x,h.y-(['barbarian','sorceress','necromancer','amazon','paladin'].includes(h.classId)?74:44),HERO_GRADES[h.grade ?? 0].color,11);}}
  ...
  const raidBoss=s.raid&&s.enemies.find(e=>e.id===s.raid.bossId);if(raidBoss){const bs=bossSprite(0),bh=bs.anchorY??bs.height-4;label(`BOSS · ${MONSTERS[raidBoss.type].name}`,raidBoss.x,raidBoss.y-(zoom<.5?bh+2:bh+32),'#ffb457',11,true);}
...
export function portrait(canvas,h,s){
 ...
 const a=appearanceOf(h,s),sprite=heroSprite(a,0,false),sw=sprite.width,sh=sprite.height,ax=sprite.anchorX??12,ay=sprite.anchorY??29;
 const size=Math.max(1,Math.floor(Math.min(w/(sw+6),he/(sh+5)))),x=Math.floor(w/2),y=Math.floor(he*.89);
 oval(c,x,y,Math.round(size*sw*.35),Math.max(1,size*2),'#15221c88');c.drawImage(sprite,x-ax*size,y-ay*size,sw*size,sh*size);
}
export function monsterPortrait(canvas,id){const c=canvas.getContext('2d');c.clearRect(0,0,canvas.width,canvas.height);c.imageSmoothingEnabled=false;const sprite=nightmareSprite(id)||monsterSprite(MONSTERS[id]);const scale=Math.min(canvas.width/sprite.width,canvas.height/sprite.height);c.drawImage(sprite,Math.floor((canvas.width-sprite.width*scale)/2),Math.floor((canvas.height-sprite.height*scale)/2),sprite.width*scale,sprite.height*scale);}
