...
import { ZONES, MONSTERS, MART, RARITIES, classOf } from './data.js';
import { statsOf, availableZone } from './engine.js';
...
export function appearanceOf(h, s) {
  const weapon=s.inventory.find(i=>i.id===h.equipment.weapon),armor=s.inventory.find(i=>i.id===h.equipment.armor),accessory=s.inventory.find(i=>i.id===h.equipment.accessory),cls=classOf(h);
  return { body:cls.id,color:palette[h.costume.palette]||armor?.appearance.color||heroIdentity(h).accent,armor:armor?.appearance.armor??-1,weapon:h.path[0]==='javelin'?'spear':cls.glyph,weaponTier:weapon?.rarity??-1,glow:accessory?.rarity??-1,tier:h.path.length,variant:Number(h.id.slice(1))%3,accent:heroIdentity(h).accent };
}
export function drawHero(c,h,s,x,y,scale=1,time=0,selected=false,pose={}) {
  ...
  if(a.glow>=1){const col=RARITIES[a.glow].color;for(let i=0;i<5;i++){const phase=time*1.5+i*1.26;rect(c,x+Math.round(Math.cos(phase)*15*scale),y-4*scale+Math.round(Math.sin(phase)*4*scale),Math.max(1,scale),Math.max(1,scale),col);}}
...
export class WorldRenderer {
 ...
 draw(s,selected,selectedZone){
  ...
  const raidBoss=s.raid&&s.enemies.find(e=>e.id===s.raid.bossId);if(raidBoss)label(`BOSS · ${MONSTERS[raidBoss.type].name}`,raidBoss.x,raidBoss.y-(zoom<.5?70:100),'#ffb457',11,true);
