...
function drawMonster(c,e,time){
 if(e.boss){const sprite=bossSprite(Math.floor(time*2+e.x)%2),x=Math.round(e.x),y=Math.round(e.y),sw=sprite.width,sh=sprite.height,top=y-(sprite.anchorY??sh-4);oval(c,x,y+2,Math.round(sw*.4),Math.round(sw*.11),'#18241d99');c.imageSmoothingEnabled=false;
  ...
  c.drawImage(sprite,x-Math.floor(sw/2),top);panel(c,x-24,top-12,49,6,'#382f31');rect(c,x-23,top-11,Math.round(47*Math.max(0,e.hp/e.maxHp)),4,e.hp<e.maxHp*.3?'#ff6a4a':'#e4b875');return;}
