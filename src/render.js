...
export class WorldRenderer {
 ...
 draw(s,selected,selectedZone){
  ...
  for(const h of s.heroes){const prev=this.previous.get(h.id),moved=prev?Math.hypot(h.x-prev.x,h.y-prev.y)>.03:false,movingUntil=moved?s.time+.2:prev?.movingUntil||0,target=s.enemies.find(e=>e.id===h.target),flip=target?target.x<h.x:moved&&prev&&Math.abs(h.x-prev.x)>.01?h.x<prev.x:prev?.flip||false;this.previous.set(h.id,{x:h.x,y:h.y,flip,movingUntil});if(!visible(h.x,h.y))continue;
   entities.push({y:h.y,draw:()=>{if(h.state==='arrive'&&h.arrivalStage===-1)l.drawImage(this.boat,h.x-82,h.y-90);for(const p of h.pets){l.globalAlpha=.7;drawMonster(l,{...p,type:p.kind==='golem'?'hulk':'skeleton',hp:1,maxHp:1},s.time);l.globalAlpha=1;}drawHero(l,h,s,Math.round(h.x),Math.round(h.y),.5,s.time,false,{moving:s.time<movingUntil,flip,attack:h.state==='hunt'&&h.attackCd>.85});if(h.hp<statsOf(h,s).hp*.8){const x=Math.round(h.x),y=Math.round(h.y-38);panel(l,x-10,y,21,4,'#2c3732');rect(l,x-9,y+1,Math.max(0,Math.round(19*h.hp/statsOf(h,s).hp)),2,h.hp<statsOf(h,s).hp*.3?'#d77c73':'#b6d185');}}});
  }
