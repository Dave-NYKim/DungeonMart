import {actProgress,finalBossUnlocked,summonActBoss,restAtOasis,ACT_BOSS_TYPES,actBossCooldownLeft} from './engine.js';
const actBossWait=zone=>{const left=actBossCooldownLeft(state,zone);return left>0?`<p class="modal-description">재소환 대기 <b>${clock(left)}</b> · 운영금으로 초기화할 수 없습니다.</p>`:'';};
import { classIcon } from './class-icons.js';
import { renameHero } from './hero-identity.js';
...
const fmt = n => n>=1e9?n.toExponential(1):Math.floor(n).toLocaleString('ko-KR');
...
const statusOf = h => h.state==='dead'?(h.soulAtClinic?`부활까지 ${Math.ceil(h.recovery)}초`:'영혼 · 회복소로 이동'):h.standby && ['recover','return'].includes(h.state) ? '마을 대기' : `${statusNames[h.state]}${h.state==='hunt'?` · ACT ${ZONES[h.zone].act}`:''}`;
...
function renderDetail() {
  ...
  $('detail-panel').innerHTML = `<div class="detail-identity"><div class="detail-top"><span class="eyebrow">ADVENTURER PROFILE</span><span>${h.path.length+1}차 직업</span></div><div class="portrait-stage"><span class="portrait-sigil" style="color:${heroIdentity(h).accent}">${icon(classOf(h).glyph)}</span><canvas id="hero-portrait" width="252" height="316" aria-label="${h.name}의 현재 장비 외형"></canvas><div class="level-orb">${h.level}</div></div><div class="detail-name"><h2 style="color:${HERO_GRADES[h.grade].color}">${h.name}</h2><button class="small-button" data-action="rename-hero">이름 변경</button><p><span style="color:${HERO_GRADES[h.grade].color}">${HERO_GRADES[h.grade].name} · 기본 능력치 ×${HERO_GRADES[h.grade].multiplier}</span></p><p class="hero-epithet">${heroIdentity(h).epithet}</p><p>${heroIdentity(h).origin}</p><p><span class="class-tag">${titleOf(h)}</span> · ${classOf(h).role}</p></div><div class="palette-row" aria-label="코스튬 색상">${[['equipment','장비 외형','#b5a57b'],['ash','잿빛','#9b9f9e'],['crimson','핏빛','#ae655c'],['forest','숲','#799c81'],['midnight','밤','#838fb6']].map(([v,n,c])=>`<button class="palette-swatch ${h.costume.palette===v?'active':''}" style="--swatch:${c}" data-action="costume" data-value="${v}" title="${n} 코스튬" aria-label="${n} 코스튬" aria-pressed="${h.costume.palette===v}"></button>`).join('')}</div></div>
...
function updateDetail() {
  ...
  $('detail-hp').style.width=`${h.hp/st.hp*100}%`; $('detail-xp').style.width=`${h.level>=levelCap(h)?100:h.xp/xpNeeded(h)*100}%`;$('detail-xp').parentElement.title=h.level>=100?'최대 레벨 100':`경험치 ${Math.floor(h.xp)} / ${xpNeeded(h)}`;
...
function renderActs() {
  ...
  const html=`${difficultyBarHTML()}<div class="zone-board" id="zone-board">${columns.map(col=>`<section class="zone-column ${col.id===-1?'town':''} ${col.open?'':'locked'} ${col.zone&&selectedZone===col.id?'active':''}" data-drop-zone="${col.id}" style="--act-color:${col.color}" aria-label="${col.name}"><header>${col.zone?`<button class="zone-title" data-action="zone" data-zone="${col.id}" aria-pressed="${selectedZone===col.id}">${col.name}</button>`:`<strong class="zone-title">${col.name}</strong>`}<small>${col.open?`${col.heroes.length}명${col.zone?` · Lv.${col.zone.level}+`:''}`:`ACT ${col.id} 보스 처치 ${icon('lock')}`}</small></header><div class="zone-heroes">${col.heroes.map(zoneChip).join('')||`<span class="zone-empty">${col.open?'여기에 놓기':'잠김'}</span>`}</div><footer>${col.zone?`<button class="small-button act-boss-button" data-action="act-boss" data-zone="${col.id}" ${!col.open||state.raid?'disabled':''}>${actProgress(state)>col.id?'보스 재도전':'액트 보스 도전'}</button><div class="spawn-control"><span>젠</span><div class="spawn-options" role="group" aria-label="ACT ${col.zone.act} 몬스터 재생성 속도">${[1,2,3,4,5].map(v=>`<button class="small-button ${state.spawnRates[col.id]===v?'active':''}" data-action="spawn-rate" data-zone="${col.id}" data-value="${v}" aria-pressed="${state.spawnRates[col.id]===v}">${v}×</button>`).join('')}</div></div><small>${MATERIALS[col.zone.material].name} · ${(1.8/state.spawnRates[col.id]).toFixed(2)}초 간격</small>`:`<small>${col.sub}</small>`}</footer></section>`).join('')}</div>`;
...
function difficultyBarHTML(){
  ...
  const stages=Array.from({length:STAGES},(_,i)=>i+1).map(st=>`<button class="diff-stage ${tier===f.tier&&st===f.stage?'active':''}" data-action="difficulty-stage" data-tier="${tier}" data-stage="${st}" ${tier>f.unlocked||tier===f.unlocked&&st>f.unlockedStage?'disabled':''} aria-label="${D.name} ${st}단계${tier>f.unlocked||tier===f.unlocked&&st>f.unlockedStage?' · 이전 단계 보스 처치 필요':''}" aria-pressed="${tier===f.tier&&st===f.stage}" title="몬스터 ×${fmt(D.mult*stageMult(st))}">${st}</button>`).join('');
  return `<div class="difficulty-bar" style="--diff-color:${DIFFICULTIES[f.tier].color}"><div class="diff-head"><span class="eyebrow">DIFFICULTY</span><strong>${DIFFICULTIES[f.tier].name} ${f.stage}단계</strong><small>몬스터 ×${fmt(DIFFICULTIES[f.tier].mult*stageMult(f.stage))} · 보상 ×${DIFFICULTIES[f.tier].reward}</small><span id="difficulty-lock" class="diff-lock"></span></div><div class="diff-tiers">${tiers}</div><div class="diff-stages" aria-label="${D.name} 단계 선택">${stages}</div><small class="diff-hint">보스 처치 → 다음 단계 · 변경 후 5분 고정</small></div>`;
}
...
function renderSkills() {
 ...
 const nodeHTML=n=>{const info=treeInfo(h,n),sk=skillById(`${n.node.id}-0`);return `<button class="tree-node ${info.state} ${n.node.id===focusId?'focus':''}" data-action="tree-node" data-id="${n.node.id}" aria-pressed="${n.node.id===focusId}" title="${n.node.name} · ${info.sub}"><span class="node-icon">${classIcon(n.node,c.id)}</span><strong>${n.node.name}</strong><small>${info.sub}</small></button>`;};
 ...
 const detail=`<div class="tree-detail"><div class="tree-detail-head"><span class="node-icon">${classIcon(f.node,c.id)}</span><div><small>${f.tier}차 · ${f.parent?f.parent.name+' → ':''}${f.node.name}</small><h3>${f.node.name}</h3><p>${f.node.theme||`${f.node.role} · ${f.node.weapon}`}${cost?` · 전직 비용 ${cost}`:''}</p></div><span class="tree-state ${finfo.state}">${finfo.sub}</span></div><div class="tree-skill-list">${f.node.skills.map((sd,idx)=>{const sk=skillById(`${f.node.id}-${idx}`);return `<button class="tree-skill" data-action="preview" data-id="${sk.id}" title="${sd.desc}">${skillIcon(sk)}<span><strong>${sd.name}</strong><small>${sd.passive?'패시브 효과':'효과 미리보기 ▷'}</small></span></button>`;}).join('')}</div></div>`;
...
function renderBestiary(){
 ...
 $('bestiary-view').innerHTML=tabs+ZONES.map(z=>`<article class="surface bestiary-act" style="--act-color:${z.color}"><div class="section-title"><div><div class="eyebrow">ACT ${z.act} · ${z.en}</div><h3 style="margin-top:8px">${z.name}</h3></div><span class="muted">Lv.${z.level}+</span></div><p>${z.theme} · 주요 재료: ${MATERIALS[z.material].name}</p><div class="monster-grid">${z.monsters.map(id=>{const m=MONSTERS[id];return `<div class="monster-entry"><canvas class="monster-portrait" data-monster="${id}" width="64" height="72" aria-hidden="true"></canvas><strong style="color:${m.color}">${m.name}</strong><small>${m.en}</small><p>${traits[m.trait]}</p></div>`;}).join('')}</div></article>`).join(''); for(const canvas of document.querySelectorAll('[data-monster]'))monsterPortrait(canvas,canvas.dataset.monster); }
function renderUI(force=false){
  ...
  const h=selectedHero();const nextSig=JSON.stringify([state.campaign,state.raid?.bossId,selected,view,h.name,h.level,h.path,h.placed,h.costume,h.skillRanks,h.skillPoints,state.itemRev,state.warehouse.length,state.fieldDrops.length,Object.values(state.drawer.gems).join(),Object.values(state.drawer.runes).join(),state.upgrades,state.materials]);
  renderDropBadges();
  ...
  renderRaid();
  $('objective').textContent=!state.crafted?'첫 영업 목표: 장비 한 개 제작하기':!state.sales?'다음 목표: 용사의 장비창에 장비 배치하기':actProgress(state)<4?`다음 목표: ACT ${actProgress(state)+1} 보스 처치`:'오늘도 던전 한가운데, 정상 영업.';
}
function renderRaid(){
  const b=state.raid&&state.enemies.find(e=>e.id===state.raid.bossId),hud=$('boss-hud'),button=$('summon-boss');
  const cd=raidCooldownLeft(state);button.hidden=!!state.raid;button.disabled=!!state.raid;button.classList.toggle('active',!!state.raid);button.classList.toggle('cooldown',!state.raid&&cd>0);button.title=state.raid?'보스 전투 중':!finalBossUnlocked(state)?'ACT 4 보스 처치 후 해금':cd>0?`봉인문 닫힘 · ${clock(cd)} 남음 · 눌러서 ${fmt(raidResetCost(state))} G로 초기화`:`보스 소환 · ${RAID_COST} G`;button.setAttribute('aria-label',button.title);const label=state.raid?'전투 중':!finalBossUnlocked(state)?'ACT 4 보스 처치':cd>0?clock(cd):'최종보스 소환';if(button._label!==label){button._label=label;button.innerHTML=icon('skull')+`<span>${label}</span>`;}
  const zonesHTML=[`<button class="zone-shortcut" data-action="camera-home" title="마을로 이동 (H)" aria-label="마을로 이동">${icon('shop')}</button>`,...ZONES.map(z=>`<button class="zone-shortcut ${availableZone(state,z.id)?'':'locked'} ${selectedZone===z.id?'active':''}" data-action="camera-zone" data-zone="${z.id}" title="ACT ${z.act} · ${z.name}${availableZone(state,z.id)?'':' · 이전 액트 보스 처치'}" aria-label="ACT ${z.act} ${z.name}">${availableZone(state,z.id)?`<b>${z.act}</b>`:icon('lock')}</button>`)].join('');
...
document.addEventListener('click',event=>{
  ...
  if(a==='rename-hero'){openModal('이름 변경','<label for="hero-name-input">새 이름</label><input id="hero-name-input" maxlength="12" autocomplete="off" placeholder="1~12자"><p id="hero-name-error" role="status"></p>','<button class="small-button" data-action="close">취소</button><button class="primary-button" data-action="save-hero-name">저장</button>');$('hero-name-input').value=h.name;}
  if(a==='save-hero-name'){const result=renameHero(h,$('hero-name-input').value);if(!result.ok){$('hero-name-error').textContent=result.message;return;}$('modal').close();signature='';renderUI(true);save();}
  ...
  if(a==='summon-boss'&&!finalBossUnlocked(state)){toast('ACT 4 보스를 먼저 처치하세요.');return;}
  if(a==='act-boss'){const zone=Number(b.dataset.zone);openModal(`ACT ${zone+1} 보스`, `<p>${MONSTERS[ACT_BOSS_TYPES[zone]].name}</p><p>모든 용사가 출전합니다.</p>${actBossWait(zone)}`, `<button class="small-button" data-action="close">취소</button><button class="primary-button" data-action="confirm-act-boss" data-zone="${zone}" ${actBossCooldownLeft(state,zone)>0?'disabled':''}>도전</button>`);}
  if(a==='confirm-act-boss'){const r=result(summonActBoss(state,Number(b.dataset.zone)));if(r.ok){$('modal').close();setView('world');renderer.focus(r.boss.x,r.boss.y-30,1);signature='';save();}}
  if(a==='oasis-rest'){const r=result(restAtOasis(state,h));if(r.ok){$('modal').close();save();}}
...
function showPoi(p){
  if(p.interaction==='act-boss'){const zone=p.zone;openModal(p.name,`<p>${MONSTERS[ACT_BOSS_TYPES[zone]].name}</p><p>${zone===3?'처치하면 최종보스 소환 해금':`처치하면 ACT ${zone+2} 해금`}</p>${actBossWait(zone)}`,`<button class="small-button" data-action="close">닫기</button><button class="primary-button" data-action="confirm-act-boss" data-zone="${zone}" ${!availableZone(state,zone)||state.raid||actBossCooldownLeft(state,zone)>0?'disabled':''}>보스 도전</button>`);return;}
  if(p.interaction==='final-boss'){document.querySelector('[data-action="summon-boss"]').click();return;}
  if(p.interaction==='rest'){openModal(p.name,'<p>선택한 용사가 이동해 무료로 회복합니다.</p>','<button class="small-button" data-action="close">닫기</button><button class="primary-button" data-action="oasis-rest">회복하러 가기</button>');return;}
