import { CLASSES, ZONES, MONSTERS, MATERIALS, RARITIES, HERO_GRADES, SLOT_NAMES, STAT_NAMES, classOf, skillsOf, titleOf } from './data.js';
import { createGame, tick, statsOf, powerOf, xpNeeded, availableZone, assignZone, recruit, setSpawnRate, craft, equip, salvage, promote, upgradeSkill, resetSkills, upgradeMart, editTown, serialize, restore } from './engine.js';
...
let craftSlot = 'weapon', craftClass = state.heroes[0].classId, craftRarity = 1, inventoryFilter = 'all';
...
+    <div class="detail-gear"><div class="detail-section-heading"><span>장착 장비</span><button class="text-button" data-action="profile-tab" data-tab="equipment">장비 관리 ↗</button></div><div class="gear-slots">${Object.entries(h.equipment).map(([slot,id])=>{const item=state.inventory.find(i=>i.id===id);return `<button class="gear-slot" data-action="gear" data-slot="${slot}"><span class="gear-icon" style="color:${item?RARITIES[item.rarity].color:''}">${icon(slotIcon(slot))}</span><span><small>${SLOT_NAMES[slot]}</small><strong style="color:${item?RARITIES[item.rarity].color:'#939c86'}">${item?item.name:'기본 장비'}</strong></span><span>›</span></button>`;}).join('')}</div></div>
...
function statText(key,value) { return `${STAT_NAMES[key]} +${['atk','def','hp'].includes(key)?Math.round(value):`${Math.round(value*100)}%`}`; }
...
function renderWorkshop() {
  const h=selectedHero(),items=state.inventory.filter(i=>inventoryFilter==='all'||i.slot===inventoryFilter);
  $('workshop-view').innerHTML=`<div class="surface"><div class="section-title"><h3>장비 제작</h3><small>재료는 공용 · 구매는 용사 골드</small></div><p>재료를 모아 장비를 만들고, ${h.name}에게 맞는 장비를 골라 주세요.</p><div class="form-row"><label>장비 종류<select id="craft-slot">${Object.entries(SLOT_NAMES).map(([k,n])=>`<option value="${k}" ${craftSlot===k?'selected':''}>${n}</option>`).join('')}</select></label><label>전용 직업<select id="craft-class" ${craftSlot!=='weapon'?'disabled':''}>${CLASSES.map(c=>`<option value="${c.id}" ${craftClass===c.id?'selected':''}>${c.name}</option>`).join('')}</select></label><label>장비 등급<select id="craft-rarity">${RARITIES.map((r,i)=>`<option value="${i}" ${craftRarity===i?'selected':''}>${r.name}</option>`).join('')}</select></label></div><div class="cost-row"><div class="costs" id="craft-cost">${costsHTML(RARITIES[craftRarity].cost)}</div><button class="primary-button" data-action="craft">${icon('axe')} 장비 제작</button></div></div>
+    <div class="section-title"><h3>마트 재고 <span class="muted">${state.inventory.length}</span></h3><select id="inventory-filter" style="width:105px;margin:0;padding:5px"><option value="all">모든 장비</option>${Object.entries(SLOT_NAMES).map(([k,n])=>`<option value="${k}" ${inventoryFilter===k?'selected':''}>${n}</option>`).join('')}</select></div><div class="inventory-grid">${items.length?items.slice().reverse().map(i=>{const owner=state.heroes.find(h=>h.equipment[i.slot]===i.id),usable=!i.classId||i.classId===h.classId;return `<article class="item-card" style="--item-color:${RARITIES[i.rarity].color}"><div class="item-card-header"><span class="gear-icon">${icon(i.slot==='weapon'?CLASSES.find(c=>c.id===i.classId).glyph:i.slot==='armor'?'armor':'gem')}</span><div><h3>${i.name}</h3><small>${RARITIES[i.rarity].name} · ${i.classId?CLASSES.find(c=>c.id===i.classId).name:'공용'} ${owner?`· ${owner.name} 장착`:''}</small></div></div><div class="item-stats">${Object.entries(i.stats).map(([k,v])=>`<div>${statText(k,v)}</div>`).join('')}</div><div class="item-actions"><button class="small-button" data-action="equip" data-id="${i.id}" ${!usable||owner?.id===h.id?'disabled':''}>${owner?.id===h.id?'장착 중':!usable?'직업 전용':i.purchased?`${h.name}에게 장착`:`구매 · ${i.price} G`}</button><button class="small-button" data-action="salvage" data-id="${i.id}" ${owner?'disabled':''}>분해</button></div></article>`;}).join(''):`<div class="empty-state">아직 진열된 장비가 없습니다.<br>위 제작소에서 첫 장비를 만들어 보세요.</div>`}</div>
+    <div class="facility-grid">${[['forge','제작대','제작 장비 기본 능력치 +12%'],['clinic','회복 시설','초당 회복 속도 +4%p']].map(([id,n,d])=>`<div class="surface"><h3>${n} <span class="muted">Lv.${state.upgrades[id]}</span></h3><p>${d}</p><button class="small-button" data-action="facility" data-id="${id}" ${state.upgrades[id]>=5?'disabled':''}>${state.upgrades[id]>=5?'최고 레벨':`시설 개선 · ${150*(state.upgrades[id]+1)} G`}</button></div>`).join('')}</div>`.replace(/^\+/gm,'');
}
function renderSkills() {
 const h=selectedHero(),c=classOf(h);
 const nodeSkills=n=>n.skills.map((s,i)=>{const sk=skillById(`${n.id}-${i}`);return `<button class="tree-skill" data-action="preview" data-id="${sk.id}" title="${s.desc}">${skillIcon(sk)}<span><strong>${s.name}</strong><small>${s.passive?'패시브 효과':'효과 미리보기 ▷'}</small></span></button>`;}).join('');
 const branchCard=(b,stage,parent)=>{const chosen=h.path.includes(b.id),eligible=stage===2?!h.path.length:h.path.length===1&&h.path[0]===parent.id,level=stage===2?20:40;return `<article class="branch-card ${chosen?'chosen':''}"><div class="branch-parent">${parent?parent.name+' → ':''}${stage}차 직업 · Lv.${level}</div><h3>${b.name} ${chosen?'✓':''}</h3><p>${b.theme}</p><div class="tree-skill-list">${nodeSkills(b)}</div><button class="small-button" data-action="promote" data-id="${b.id}" ${!eligible||h.level<level?'disabled':''}>${chosen?'현재 선택한 계열':!eligible?'다른 계열 · 미리보기 가능':h.level<level?`Lv.${level}에 전직`:'이 계열로 전직'}</button></article>`;};
 const scroll=$('skills-view').scrollTop;
 $('skills-view').innerHTML=`<div class="skill-studio"><section id="skill-preview" class="skill-preview" aria-label="스킬 효과 미리보기"></section><div class="skill-tree-content"><div class="surface"><h3>${h.name} · ${titleOf(h)}</h3><p>아이콘을 눌러 아직 배우지 않은 스킬도 미리 보세요.<br>전직은 Lv.20 / Lv.40에 선택합니다. 미리보기는 재료·포인트를 사용하지 않습니다.</p></div><div class="tree-label">기본 직업 · ${c.name}</div><div class="branch-card chosen"><h3>${c.name}</h3><p>${c.role} · ${c.weapon}</p><div class="tree-skill-list">${nodeSkills(c)}</div></div><div class="tree-label">2차 전문화 · 철 조각 35 / 마력석 12</div><div class="tree-grid">${c.branches.map(b=>branchCard(b,2)).join('')}</div><div class="tree-label">3차 최종 전직 · 마력석 25 / 영혼 결정 15</div><div class="tree-grid">${c.branches.flatMap(b=>b.children.map(n=>branchCard(n,3,b))).join('')}</div><div class="section-title" style="margin-top:24px"><h3>배운 스킬 · ${h.skillPoints} P</h3><button class="text-button" data-action="reset-skills">포인트 초기화 · 무료</button></div>${skillsOf(h).map(sk=>`<div class="skill-row"><button class="skill-mini" data-action="preview" data-id="${sk.id}" aria-label="${sk.name} 미리보기">${skillIcon(skillById(sk.id))}</button><div class="skill-info"><h4>${sk.name}<span>Lv.${sk.rank} / 5</span></h4><p>${sk.desc}</p></div><button class="icon-button" data-action="skill" data-id="${sk.id}" aria-label="${sk.name} 강화" title="${sk.rank} 포인트 사용" ${h.skillPoints<sk.rank||sk.rank>=5?'disabled':''}>+</button></div>`).join('')}</div></div>`;
...
const traits={coward:'동료가 쓰러지면 잠시 후퇴',revive:'근처의 시체를 되살리는 원거리 시전자',tough:'높은 체력 · 느린 이동',melee:'가까운 용사를 추격하는 근접 공격',charged:'피격 시 일정 확률로 전격 반격',charge:'빠른 접근과 연속 공격',poison:'원거리 독 공격 · 지속 피해',fire:'주기적인 범위 화염 공격',drain:'원거리 공격으로 생명력 흡수',curse:'공격 시 일시적으로 용사 약화',spawn:'주기적으로 작은 하수인 생성'};
...
function renderUI(force=false){
  ...
  const h=selectedHero();const nextSig=JSON.stringify([selected,view,h.level,h.path,h.equipment,h.costume,h.skillRanks,h.skillPoints,state.inventory.map(i=>[i.id,i.purchased]),state.upgrades]);
  ...
  if((view==='workshop'||$('hero-modal').open&&profileTab==='equipment')&&$('craft-cost'))$('craft-cost').innerHTML=costsHTML(RARITIES[craftRarity].cost);
  $('objective').textContent=!state.crafted?'첫 영업 목표: 장비 한 개 제작하기':!state.sales?'다음 목표: 용사에게 장비 구매·장착하기':!state.heroes.some(h=>h.level>=8)?'다음 목표: 용사 Lv.8 달성 · ACT II 해금':'오늘도 던전 한가운데, 정상 영업.';
...
document.addEventListener('click',event=>{
  ...
  if(a==='gear'){craftSlot=b.dataset.slot;inventoryFilter=b.dataset.slot;craftClass=h.classId;setProfileTab('equipment');}
  if(a==='craft')result(craft(state,craftSlot,craftClass,craftRarity));
  if(a==='equip')result(equip(state,h,b.dataset.id));
  if(a==='salvage')result(salvage(state,b.dataset.id));
...
document.addEventListener('change',event=>{const el=event.target;if(el.id==='craft-slot')craftSlot=el.value;if(el.id==='craft-class')craftClass=el.value;if(el.id==='craft-rarity')craftRarity=Number(el.value);if(el.id==='inventory-filter')inventoryFilter=el.value;if(['craft-slot','craft-class','craft-rarity','inventory-filter'].includes(el.id))renderWorkshop();});
