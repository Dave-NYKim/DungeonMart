import {actProgress,finalBossUnlocked,summonActBoss,restAtOasis,ACT_BOSS_TYPES} from './engine.js';
...
document.addEventListener('click',event=>{
  ...
  if(a==='act-boss'){const zone=Number(b.dataset.zone);openModal(`ACT ${zone+1} 보스`, `<p>${MONSTERS[ACT_BOSS_TYPES[zone]].name}</p><p>모든 용사가 출전합니다.</p>`, `<button class="small-button" data-action="close">취소</button><button class="primary-button" data-action="confirm-act-boss" data-zone="${zone}">도전</button>`);}
...
function showPoi(p){
  if(p.interaction==='act-boss'){const zone=p.zone;openModal(p.name,`<p>${MONSTERS[ACT_BOSS_TYPES[zone]].name}</p><p>${zone===3?'처치하면 최종보스 소환 해금':`처치하면 ACT ${zone+2} 해금`}</p>`,`<button class="small-button" data-action="close">닫기</button><button class="primary-button" data-action="confirm-act-boss" data-zone="${zone}" ${!availableZone(state,zone)||state.raid?'disabled':''}>보스 도전</button>`);return;}
