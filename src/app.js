...
function renderRaid(){
  ...
  const cd=raidCooldownLeft(state);button.hidden=!!state.raid;button.disabled=!!state.raid;button.classList.toggle('active',!!state.raid);button.classList.toggle('cooldown',!state.raid&&cd>0);button.title=state.raid?'보스 전투 중':cd>0?`봉인문 닫힘 · ${clock(cd)} 남음 · 눌러서 ${fmt(raidResetCost(state))} G로 초기화`:`보스 소환 · ${RAID_COST} G`;button.setAttribute('aria-label',button.title);const label=state.raid?'전투 중':cd>0?clock(cd):'보스 소환';if(button._label!==label){button._label=label;button.innerHTML=icon('skull')+`<span>${label}</span>`;}
