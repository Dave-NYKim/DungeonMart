...
export function skillIcon(sk){const family=skillFamily(sk),hash=[...(sk.id||sk.name)].reduce((n,c)=>n+c.charCodeAt(0),0);return `<svg class="skill-art" viewBox="0 0 36 36" aria-hidden="true" style="--skill-color:${colors[family]}"><rect x="1" y="1" width="34" height="34" rx="4" fill="#142920" stroke="${colors[family]}"/><path d="${motifs[family]}" transform="translate(2 2)" fill="${['fire','blood','lightning','holy'].includes(family)?colors[family]+'44':'none'}" stroke="${colors[family]}" stroke-width="2" stroke-linejoin="bevel"/>${Array.from({length:1+hash%3},(_,i)=>`<rect x="${5+i*5}" y="30" width="3" height="2" fill="#fff0bc"/>`).join('')}${sk.passive?'<path d="M27 3L33 9L27 15L21 9Z" fill="#f6e4b4" stroke="#294537"/>':''}</svg>`;}
