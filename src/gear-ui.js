...
export function installGearUI(host, ctx, mode = 'hero') {
  ...
  const priceHTML = (item, h) => `<span class="item-sale-price ${item.purchased ? 'purchased' : ''}">${item.purchased ? '구매 완료' : `판매가 ${ctx.fmt(salePrice(item, h))} G`}</span>`;
  ...
  function detailHTML(h, s) {
    ...
    const b = itemBase(item), loc = locationOf(item.id), placedHere = h && h.placed.some(p => p.id === item.id), lines = itemLines(item);
