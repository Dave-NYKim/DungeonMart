...
export function installGearUI(host, ctx, mode = 'hero') {
  ...
  function warehouseHTML(h, s) {
    ...
    const bulk = mode !== 'hero' ? `<div class="salvage-row"><span class="muted">전체 분해:</span>${['normal', 'magic', 'rare'].map(g => `<button class="small-button" data-gear="salvage-all" data-grade="${g}" ${counts[g] ? '' : 'disabled'}>${GRADES[g].name} ${counts[g]}개</button>`).join('')}<span class="muted">세트·유니크 분해 → 유물의 정수</span></div>` : '';
    return `<div class="wh-head"><h4>마을 창고 <small>${warehouseUsed(s)} / ${warehouseCapacity(s)}개${mode === 'hero' ? ` · ${esc(h.name)}이(가) 쓸 수 있는 장비만 표시` : ''}</small></h4><div class="wh-controls"><select id="wh-filter">${[['all', '전체'], ['weapon', '무기·보조'], ['armor', '방어구'], ['accessory', '장신구'], ['sockets', '홈 있음'], ['rare', '세트·유니크·진언']].map(([v, n]) => `<option value="${v}" ${ui.filter === v ? 'selected' : ''}>${n}</option>`).join('')}</select><select id="wh-sort">${[['grade', '등급순'], ['ilvl', '레벨순'], ['price', '가격순']].map(([v, n]) => `<option value="${v}" ${ui.sort === v ? 'selected' : ''}>${n}</option>`).join('')}</select></div></div>${bulk}<div class="wh-list" data-wh-drop>${warehouseRows(h, s, list) || `<div class="empty-state">${mode === 'hero' ? '이 용사가 쓸 수 있는 장비가 창고에 없습니다. 제작소에서 만들거나 사냥 전리품을 기다리세요.' : '창고가 비어 있습니다. 제작소에서 장비를 만들거나, 사냥에서 세트·유니크 빛기둥을 획득하세요.'}</div>`}</div>`;
  }
  function detailHTML(h, s) {
    const item = s.items[ui.sel]; if (!item) return `<div class="gear-detail empty"><p>${mode === 'hero' ? '장비 선택 → 배치' : '장비를 눌러 상세 보기'}</p></div>`;
    ...
    const actions = mode === 'hero'
      ? (!placedHere ? `<button class="primary-button" data-gear="auto" data-id="${item.id}" ${usable(item, h) && fitsSomewhere(item, h) && loc.text === '창고' ? '' : 'disabled'}>배치</button>` : `<button class="small-button" data-gear="unplace" data-id="${item.id}">해제</button><button class="small-button" data-gear="rotate" data-id="${item.id}" ${item.w === item.h ? 'disabled' : ''}>회전</button>`)
