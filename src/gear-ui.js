...
import { BASES, GRADES, GEMS, GEM_QUALITY, RUNES, RUNE_LIST, RUNE_TIER_COLORS, SETS, MANTRAS, uniqueById, itemBase, itemPassives, weightOf, gradeColor, displayName, describeStat, insertName, insertColor, effectiveGrid, dims, levelReq, TIER_NAMES, ZONE_NAMES, PASSIVE_NAMES, RECIPES, fits } from './items.js';
...
export function itemTitle(item) { return `${displayName(item)} · ${GRADES[item.grade].name} ${TIER_NAMES[item.tier]} ${itemBase(item).names[item.tier - 1]}\n${itemLines(item).map(l => l.text).join('\n')}`; }
...
export function installGearUI(host, ctx, mode = 'hero') {
  const ui = { sel: null, insertSel: null, filter: 'all', sort: 'grade', craftCat: 'weapon', craftBase: 'sword1h', craftGrade: 'normal', craftTier: 1 };
  ...
  // ---- 통합 인벤토리: 하나의 틀 안에 무기 · 방어구 · 장신구 구간
  function inventoryHTML(h, s) {
    const grid = effectiveGrid(h), bonus = gearBonus(h, s);
    const region = zone => {
      const g = grid[zone], cells = [];
      for (let y = 0; y < g.h; y++) for (let x = 0; x < g.w; x++) cells.push(`<div class="grid-cell" data-zone="${zone}" data-x="${x}" data-y="${y}"></div>`);
      const items = h.placed.filter(p => p.zone === zone).map(p => { const item = s.items[p.id]; if (!item) return ''; const d = dims({ ...item, rotated: p.rotated }), a = bonus.act.get(p.id), col = gradeColor(item); return `<div class="grid-item ${a?.active ? '' : 'inactive'} ${ui.sel === p.id ? 'selected' : ''} ${item.mantra ? 'mantra' : ''}" data-drag-id="${p.id}" data-gear="select" data-id="${p.id}" style="--x:${p.x};--y:${p.y};--w:${d.w};--h:${d.h};--item-color:${col}" title="${esc(itemTitle(item))}${a?.reason ? `\n비활성: ${a.reason}` : ''}${a?.secondWeapon ? '\n보조 무기' : ''}"><span class="grid-icon">${ctx.icon(iconFor(item))}</span>${d.w * d.h >= 3 ? `<span class="grid-label">${esc(displayName(item))}</span>` : ''}${item.sockets ? `<span class="grid-sockets">${item.inserts.map(k => `<i style="background:${k ? insertColor(k) : '#3a3f36'}"></i>`).join('')}</span>` : ''}${a?.reason ? `<span class="grid-reason">${esc(a.reason)}</span>` : ''}</div>`; }).join('');
      const info = zone === 'weapon' ? (() => { let hands = 0; for (const p of h.placed) if (p.zone === 'weapon' && bonus.act.get(p.id)?.active) hands += itemBase(s.items[p.id]).hands === 2 ? 2 : 1; return `손 ${hands}/2`; })() : zone === 'armor' ? `${['head', 'body', 'hands', 'feet', 'waist'].filter(part => h.placed.some(p => p.zone === 'armor' && bonus.act.get(p.id)?.active && itemBase(s.items[p.id]).part === part)).length}/5 부위` : `반지 ${h.placed.filter(p => p.zone === 'accessory' && bonus.act.get(p.id)?.active && itemBase(s.items[p.id]).type === 'ring').length}/2`;
      return { html: `<div class="inventory-region grid-zone" data-grid-zone="${zone}" data-label="${ZONE_NAMES[zone].replace('칸', '')}" style="--w:${g.w};--h:${g.h}">${cells.join('')}${items}</div>`, info: `<span><b>${ZONE_NAMES[zone].replace('칸', '')}</b> ${g.w}×${g.h} · ${info}</span>` };
    };
    const regions = ['weapon', 'armor', 'accessory'].map(region);
    return `<div class="inventory-frame">${regions.map(r => r.html).join('')}</div><div class="inventory-info">${regions.map(r => r.info).join('')}</div>`;
  }
  function heroPanel(h, s) {
    const st = statsOf(h, s), bonus = gearBonus(h, s), load = Math.round(st.load * 100), grid = effectiveGrid(h);
  ...
  function warehouseRows(h, s, list) {
    return list.map(i => { const b = itemBase(i), fit = h ? usable(i, h) && fitsSomewhere(i, h) : false; return `<div class="wh-item ${ui.sel === i.id ? 'selected' : ''}" data-drag-id="${i.id}" data-gear="select" data-id="${i.id}" style="--item-color:${gradeColor(i)}" title="${esc(itemTitle(i))}"><span class="wh-icon">${ctx.icon(iconFor(i))}</span><div class="wh-text"><strong>${esc(displayName(i))}</strong><small>${GRADES[i.grade].name} · ${TIER_NAMES[i.tier]} ${b.names[i.tier - 1]} · ${i.w}×${i.h} · ${weightOf(i)}kg · ${classNames(b)}${i.sockets ? ` · 홈 ${i.inserts.filter(Boolean).length}/${i.sockets}` : ''}${i.purchased ? '' : ` · ${ctx.fmt(h ? Math.round(i.price * (1 + statsOf(h, s).price)) : i.price)} G`}</small></div>${mode === 'hero' ? `<button class="small-button" data-gear="auto" data-id="${i.id}" ${fit ? '' : 'disabled'} title="${fit ? '빈 자리에 자동 배치' : h.level < levelReq(i) ? `Lv.${levelReq(i)} 필요` : '빈 자리 없음'}">배치</button>` : `<button class="small-button danger" data-gear="salvage" data-id="${i.id}" title="${Object.entries(salvageValue(i)).map(([k, v]) => `${MAT[k]} ${v}`).join(', ')}">분해</button>`}</div>`; }).join('');
  }
  function warehouseHTML(h, s) {
    let list = s.warehouse.map(id => s.items[id]).filter(Boolean);
    if (mode === 'hero') list = list.filter(i => usable(i, h));
    if (ui.filter === 'sockets') list = list.filter(i => i.sockets > 0); else if (ui.filter === 'rare') list = list.filter(i => ['set', 'unique'].includes(i.grade) || i.mantra); else if (ui.filter !== 'all') list = list.filter(i => itemBase(i).zone === ui.filter);
    ...
    const counts = { normal: 0, magic: 0, rare: 0 }; for (const id of s.warehouse) { const g = s.items[id]?.grade; if (g in counts) counts[g]++; }
    const bulk = mode === 'workshop' ? `<div class="salvage-row"><span class="muted">전체 분해:</span>${['normal', 'magic', 'rare'].map(g => `<button class="small-button" data-gear="salvage-all" data-grade="${g}" ${counts[g] ? '' : 'disabled'}>${GRADES[g].name} ${counts[g]}개</button>`).join('')}<span class="muted">세트·유니크는 하나씩 분해하며 유물의 정수를 줍니다.</span></div>` : '';
    return `<div class="wh-head"><h4>마을 창고 <small>${warehouseUsed(s)} / ${warehouseCapacity(s)}칸${mode === 'hero' ? ` · ${esc(h.name)}이(가) 쓸 수 있는 장비만 표시` : ''}</small></h4><div class="wh-controls"><select id="wh-filter">${[['all', '전체'], ['weapon', '무기·보조'], ['armor', '방어구'], ['accessory', '장신구'], ['sockets', '홈 있음'], ['rare', '세트·유니크·진언']].map(([v, n]) => `<option value="${v}" ${ui.filter === v ? 'selected' : ''}>${n}</option>`).join('')}</select><select id="wh-sort">${[['grade', '등급순'], ['ilvl', '레벨순'], ['price', '가격순']].map(([v, n]) => `<option value="${v}" ${ui.sort === v ? 'selected' : ''}>${n}</option>`).join('')}</select></div></div>${bulk}<div class="wh-list" data-wh-drop>${warehouseRows(h, s, list) || `<div class="empty-state">${mode === 'hero' ? '이 용사가 쓸 수 있는 장비가 창고에 없습니다. 제작소에서 만들거나 사냥 전리품을 기다리세요.' : '창고가 비어 있습니다. 용사가 귀환하면 전리품이 들어오고, 위 제작소에서 만들 수도 있습니다.'}</div>`}</div>`;
  }
  function detailHTML(h, s) {
    const item = s.items[ui.sel]; if (!item) return `<div class="gear-detail empty"><p>${mode === 'hero' ? '장비를 선택하면 상세 정보가 표시됩니다. 창고의 장비를 인벤토리 구간으로 끌어다 놓거나 <b>배치</b>를 누르세요. 놓는 순간 효과가 적용되고 창고에서 사라지며, 해제하면 다시 창고로 돌아갑니다.' : '창고의 장비를 선택하면 상세 정보와 분해·조합·박음돌 메뉴가 표시됩니다.'}</p></div>`;
    const b = itemBase(item), loc = locationOf(item.id), placedHere = h && h.placed.some(p => p.id === item.id), lines = itemLines(item), st = h ? statsOf(h, s) : null;
    const setBlock = item.setId && h ? (() => { const set = SETS[item.setId], have = new Set(h.placed.map(p => s.items[p.id]).filter(i => i?.setId === item.setId).map(i => i.setIndex)), n = gearBonus(h, s).sets[item.setId] || 0; return `<div class="set-block"><h5>${set.name} 세트 · ${n}/${set.pieces.length}</h5><ul>${set.pieces.map((p, i) => `<li class="${have.has(i) ? 'have' : ''}">${have.has(i) ? '✓' : '·'} ${BASES[p.baseKey].names[0]}</li>`).join('')}</ul><ul>${set.bonuses.map((bn, i) => bn ? `<li class="${n >= i ? 'have' : ''}">${i}부위 · ${[...Object.entries(bn.stats || {}).map(([k, v]) => describeStat(k, v)), ...(bn.note ? [bn.note] : [])].filter(Boolean).join(', ')}</li>` : '').join('')}</ul></div>`; })() : '';
    const recipes = [];
    if (mode === 'workshop') {
    ...
    const actions = mode === 'hero'
      ? (!placedHere ? `<button class="primary-button" data-gear="auto" data-id="${item.id}" ${usable(item, h) && fitsSomewhere(item, h) && loc.text === '창고' ? '' : 'disabled'}>${esc(h.name)}에게 배치</button>` : `<button class="small-button" data-gear="unplace" data-id="${item.id}">해제 · 창고로</button><button class="small-button" data-gear="rotate" data-id="${item.id}" ${item.w === item.h ? 'disabled' : ''}>회전</button>`)
  ...
  function render() {
    ...
    if (mode === 'hero') host.innerHTML = `<div class="gear-layout hero-mode"><section class="gear-left">${heroPanel(h, s)}${inventoryHTML(h, s)}<p class="gear-hint">창고에서 구간으로 끌어다 놓거나 <b>배치</b>를 누르세요. 무기는 무기 구간, 방어구는 방어구 구간, 장신구는 장신구 구간에만 들어갑니다. 드래그 중 R 키로 회전. 회색 빗금은 조건 미달로 효과가 없는 장비입니다. 제작·분해·박음돌은 하단 메뉴의 <b>제작소</b>에서.</p></section><section class="gear-right">${warehouseHTML(h, s)}</section><section class="gear-bottom">${detailHTML(h, s)}</section></div>`;
  ...
  host.addEventListener('click', e => {
    ...
    if (a === 'salvage-all') { ui.sel = null; ctx.result(salvageAll(s, b.dataset.grade)); return; }
  ...
  // ---- 드래그 앤 드롭(hero 모드): 격자 아이템/창고 행 → 구간 셀, 또는 창고 목록으로 해제
