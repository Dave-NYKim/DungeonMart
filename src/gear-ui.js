...
export function installGearUI(host, ctx, mode = 'hero') {
  ...
  function detailHTML(h, s) {
    const item = s.items[ui.sel]; if (!item) return `<div class="gear-detail empty"><p>${mode !== 'workshop' ? '장비를 선택하면 상세 정보가 표시됩니다. 창고의 장비를 인벤토리 구간으로 끌어다 놓거나 <b>배치</b>를 누르세요. 놓는 순간 효과가 적용되고 창고에서 사라지며, 해제하면 다시 창고로 돌아갑니다.' : '창고의 장비를 선택하면 상세 정보와 분해·조합·박음돌 메뉴가 표시됩니다.'}</p></div>`;
