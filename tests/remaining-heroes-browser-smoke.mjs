...
try {
 ...
 for(const classId of ['necromancer','amazon','paladin'])for(const [width,height] of [[390,844],[360,640]]){
 await send('Emulation.setDeviceMetricsOverride',{width,height,deviceScaleFactor:1,mobile:true});
 await send('Page.navigate',{url:BASE});await sleep(1500);
 await until(`import('./src/remaining-hero-art.js').then(m=>m.remainingHeroArtStatus()['${classId}'].ready)`);
 console.log(await evaluate(`(async()=>{const m=await import('./src/remaining-hero-art.js');const dirs=[];for(const motion of ['walk'])for(let d=0;d<8;d++){let a=m.remainingHeroSprite('${classId}',0,d,motion),b=m.remainingHeroSprite('${classId}',2,d,motion);if(!a||a.width!==64||a.toDataURL()===b.toDataURL())throw Error('Invalid animation frames '+d);if(a.getContext('2d').getImageData(0,0,1,1).data[3]!==0)throw Error('Opaque background');dirs.push(a.width);}if(document.documentElement.scrollWidth>innerWidth)throw Error('Overflow');return m.remainingHeroArtStatus()['${classId}']})()`));
 await evaluate(`(async()=>{const e=await import('./src/engine.js'),r=await import('./src/render.js'),s=e.createGame(),h=e.makeHero(s,'${classId}');const c=document.createElement('canvas');c.width=252;c.height=316;c.style.cssText='position:fixed;top:120px;left:30px;background:#29372e;z-index:999';document.body.append(c);r.portrait(c,h,s);})()`);await screenshot(classId+'-mobile-'+width);
 }
 assert.deepEqual(errors,[]);console.log('Remaining heroes mobile checks passed');
} finally {ws.close();}
