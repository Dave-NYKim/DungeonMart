...
try {
 ...
 await until(`import('./src/barbarian-art.js').then(m=>m.barbarianArtStatus().loaded===24)`);
 console.log(await evaluate(`(async()=>{const m=await import('./src/barbarian-art.js');const dirs=[];for(const motion of ['walk','attack'])for(let d=0;d<8;d++){let a=m.barbarianSprite(0,d,motion),b=m.barbarianSprite(1,d,motion);if(!a||a.width!==(motion==='attack'?92:64)||a.toDataURL()===b.toDataURL())throw Error('Invalid animation frames '+d);dirs.push(a.width);}if(document.documentElement.scrollWidth>innerWidth)throw Error('Overflow');return m.barbarianArtStatus()})()`));
 await evaluate(`(async()=>{const e=await import('./src/engine.js'),r=await import('./src/render.js'),s=e.createGame(),h=e.makeHero(s,'barbarian');const c=document.createElement('canvas');c.width=252;c.height=316;c.style.cssText='position:fixed;top:120px;left:30px;background:#29372e;z-index:999';document.body.append(c);r.portrait(c,h,s);})()`);await screenshot('barbarian-mobile-'+width);
 }
 assert.deepEqual(errors,[]);console.log('Barbarian mobile checks passed');
} finally {ws.close();}
